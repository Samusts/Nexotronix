...
const OFFLINE_URL = '/PKSamustsNexotronix/';
const STATIC_URLS = [
  '/PKSamustsNexotronix/',
  '/PKSamustsNexotronix/index.html',
  '/PKSamustsNexotronix/manifest.json',
];
...
self.addEventListener('push', e => {
  let data = { title: 'Nexotronix', body: 'You have a new update!', icon: '/PKSamustsNexotronix/icon.png', url: '/PKSamustsNexotronix/' };
  ...
  const options = {
    body: data.body,
    icon: data.icon || '/PKSamustsNexotronix/icon.png',
    badge: data.badge || '/PKSamustsNexotronix/icon.png',
    tag: data.tag || 'elitetech-notif',
    renotify: true,
    requireInteraction: false,
    silent: false,
    vibrate: [200, 100, 200],
    data: { url: data.url || '/PKSamustsNexotronix/' },
    actions: [
...
self.addEventListener('notificationclick', e => {
  ...
  const targetUrl = e.notification.data?.url || '/PKSamustsNexotronix/';
    ...
    clients.matchAll({ type: 'window', includeUncontrolled: true })
      .then(clientList => {
        ...
        for (const client of clientList) {
          if (client.url.includes('PKSamustsNexotronix') && 'focus' in client) {
            client.focus();
