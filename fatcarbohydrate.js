...
/* _genOTP removed — replaced by real SMS OTP */
...
/* _applyMath removed — replaced by real SMS OTP */

/* generateOTPChallenge removed — replaced by real SMS OTP */

/* renderOTPChallenge removed — replaced by real SMS OTP */
...
/* verifyOTPInput removed — replaced by real SMS OTP */



/* ==============================================================
   REAL SMS OTP — Replaces the old color-puzzle OTP system
   ============================================================== */
let _erpPreAuthToken = null;
let _erpOtpTimer = null;
...
async function sendAdminOTP() {
  try {
    ...
    const data = await res.json();
    if (res.ok) {
      _erpPreAuthToken = data.preAuthToken || null;
      startERPOTPCountdown();
      const resendBtn = document.getElementById('otp-resend-btn');
      if (resendBtn) resendBtn.disabled = true;
      setTimeout(() => { if (resendBtn) resendBtn.disabled = false; }, 120000);
    } else {
      document.getElementById('otp-error').textContent = data.error || 'Could not send OTP';
    }
  } catch(e) {
    document.getElementById('otp-error').textContent = '⚠️ Could not reach server';
  }
}
...
function startERPOTPCountdown() {
  clearInterval(_erpOtpTimer);
  const el = document.getElementById('otp-countdown');
  if (!el) return;
  el.style.color = '';
  let seconds = 120;
  _erpOtpTimer = setInterval(() => {
    seconds--;
    ...
    if (seconds <= 0) {
      clearInterval(_erpOtpTimer);
      el.textContent = 'Expired — tap Resend';
      el.style.color = 'var(--red)';
    }
  }, 1000);
}
...
/* refreshOTPChallenge removed — replaced by real SMS OTP */

/* submitOTP removed — replaced by real SMS OTP */

/* cancelOTP removed — replaced by real SMS OTP */
